        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for instrument in instruments:
                row = [
                    ...
                    str(instrument.acquisition_cost) if instrument.acquisition_cost else "",
                    str(instrument.current_value) if instrument.current_value else "",
                    str(instrument.replacement_cost) if instrument.replacement_cost else "",
                    "YES" if instrument.patreon_active else "NO",
                    str(instrument.patreon_amount) if instrument.patreon_amount else "",
