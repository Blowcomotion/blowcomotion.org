...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for member in inactive_members:
            ...
            try:
                # get member's gigo ID
                gigo_id = member.get_gigo_id()
                
            except Exception as exc:
                self.stdout.write(
                    self.style.ERROR(f'Error retrieving Gigo ID for member {member.full_name}: {exc}')
                )
                continue
            if gigo_id:
                try:
                    # mark member as occasional in gigo
                    ...
                    if not band_id:
                        self.stdout.write(
                            self.style.WARNING(f'GIGO_BAND_ID not configured. Skipping Gigo update for {member.full_name}.')
                        )
                        continue
                    ...
                    # Toggle member to occasional status in GO3
                    # Since this is a toggle, we need to check the result and toggle again if needed
                    ...
                    if response and 'is_occasional' in response:
                        # Check if member is now occasional
                        if response['is_occasional']:
                            # Successfully marked as occasional
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f'Marked member {member.full_name} as occasional in Gigo')
                            )
                        else:
                            # Member was already occasional, so toggle made them regular
                            # Toggle again to make them occasional
                            self.stdout.write(
                                self.style.NOTICE(
                                    f'Member {member.full_name} was already occasional in Gigo, toggling back...')
                            )
                            response2 = make_gigo_api_request(endpoint, method='PATCH')
                            if response2 and response2.get('is_occasional'):
                                self.stdout.write(
                                    self.style.SUCCESS(
                                        f'Confirmed member {member.full_name} is occasional in Gigo')
                                )
                            else:
                                self.stdout.write(
                                    self.style.WARNING(
                                        f'Could not verify occasional status for {member.full_name} in Gigo')
                                )
                    else:
                        self.stdout.write(
                            self.style.WARNING(f'Could not update occasional status for {member.full_name} in Gigo')
                        )
                    
                except Exception as exc:
                    self.stdout.write(
                        self.style.ERROR(f'Error marking member {member.full_name} as occasional in Gigo: {exc}')
                    )
            else:
                self.stdout.write(
                    self.style.WARNING(f'No Gigo ID found for member {member.full_name}. Skipping Gigo update.')
                )
