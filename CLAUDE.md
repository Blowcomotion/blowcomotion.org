...
**Hybrid app layout:** domain apps (`gigs`, `attendance`, `charts`, `instruments`, `members`) hold views, forms, urls, templates, tests, and management commands for their domain. ALL models for these five apps live in the `blowcomotion` app, in the `blowcomotion/models/` package — this is legacy from the #312 split, done to avoid migration/content-type risk on existing production data. These five apps must not gain a `models.py` or `migrations/` of their own. **New apps for new domains are not bound by this** — they may define their own `models.py` and migrations with their own tables, since there's no existing production data to migrate around. `blowcomotion/` itself also keeps StreamField blocks (`blowcomotion/blocks/`), the Wagtail admin wiring (`snippet_viewsets.py`, `chooser_viewsets.py`, `wagtail_hooks.py`), and cross-domain/infra management commands.
